...
\begin{consigna1}
El proyecto nace de la desafortunada esperiencia de que una mascota se escape o se estravie. Esta es una situación deseperante, donde no se sabe por donde comenzar la búsqueda y la deseperación que aumenta con el pasar de los minutos. De esta problemática da surge la pregunta de como puede ayudar la tecnología para estos casos. Es por esto que se propone realizar el prototipo de un collar para mascotas que cuente con un módulo GPS para geolocalizar a los animales y conectividad a la red celular para reportar su posición.

Actualmente, hay competidores que ya ofrecen un producto similares pero que no logran imponerse en el mercado. Se pueden encontrar alternativas importadas de productos similares, pero ninguna de ellas esta pensada para el mercado argentino y sus necesidades. Las opciones existen que abordan esta problemática tienen 2 problemas principales que podemos identificar en las siguientes categorías:

\begin{itemize}
	\item Las que son muy costosas. Hay soluciones cuyo costo de entrada es muy elevado o que requierern una suscripcion mensual dolarizada ya que se trata de un producto sin representación en el país.
	\item Las que depende de tecnología externa. Muchos productos constan de un collar que simplemente transporta un Airtag, dependiendo de la marca Apple tanto para su posicionamiento como para poder localizar los dispositivos.
\end{itemize}

El objetivo del proyecto es brindar una respuesta a esta problemática, ofreciendo una producto nacional, con disponibilida y soporte a un precio competitivo para el seguimiento de las mascotas. En el contexto del CESE el alcance en este proyecto contempla solo el desarrollo de un sistema embebido usando una placa de desarrollo. Se realizará el firmware necesario y la integración con los diferentes periféricos externos y un dashboard básico para interactuar con el dispositivos. El desarrollo de una placa de hardware y el desarrollo de una app mobile quedarán pendientes para siguientes etapas del proyecto. 
...
El proyecto está enfocado en aquellas personas que aman a sus mascotas, las consideran parte de su familia y están dispuestos a adquirir un dispositivo para encontrarlos rapidamente en caso de que se escapen o alguien los robe. El producto debe ser accesible para que pueda llegar a una gran cantidad de usuarios, con la condición de que una vez adquirido el producto solo deberá a pagar un abono de conexión a la red celular, tal como lo haría con un teléfono celular.

Uno de los principales desafíos del proyecto consiste en lograr un equilibrio adecuado entre autonomía, funcionalidad y costo. En particular, será necesario diseñar un dispositivo con suficiente duración de batería para operar durante períodos prolongado. A esto se suma la necesidad de asegurar una cobertura celular en distintos entornos. Además, el collar deberá mantener un tamaño y peso reducidos para no afectar la comodidad del animal y ofrecer resistencia al uso diario. Por último, se debe alcanzar un costo final competitivo que lo vuelva accesible para el mercado objetivo.

En la figura \ref{fig:diagBloques} podemos ver los componentes principales que tendrá el sistema. Para lograr la comunicación con el usuario tendrá 2 formas, por un lado un led de status mediante el cual verá el estado del sistema y por otro lado una comunicaión de datos mediante un módulo 4G. Para la geolocalización del dispositivo se puede ver que el sistema tendrá un modulo especifico para ello. Otro módulo a destacar es la inclusion de una tarjeta SD para almacenar los datos y reportarlos luego ante un bache de conexión a la red. Por último tenemos una batería para alimentar el sistema.
...
\begin{figure}[htpb]
\centering 
\includegraphics[width=.65\textwidth]{./Figuras/_diagBloques.png}
\caption{Diagrama en bloques del sistema.}
\label{fig:_diagBloques}
\end{figure}
